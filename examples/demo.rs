...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut obj4 = Instance::new(
        app.assets.add_mesh(Mesh::create_cube()),
    ...
    //
    // 64 текселя на две клетки, хотя сам рисунок задаётся и четырьмя. Лишнее
    // разрешение тут ради мип-пирамиды: у картинки 4x4 в ней всего три уровня
    // (4, 2, 1), и переход от резкого к серому происходит рывком уже в паре
    // метров от камеры. Плюс эта же картинка идёт запасной вместо фотографии,
    // а там она попадёт под билинейное растяжение
    let checker_image = Texture::checker(64, 2, [230, 230, 230, 255], [40, 40, 60, 255]);

    // Путь — от корня ассетов, а не от того места, откуда запустили процесс:
    // под `cargo run` это корень проекта, у собранной игры — каталог рядом
    // с исполняемым файлом.
    //
    // Проверять, нашёлся ли файл, не надо: не нашёлся — движок подставит
    // малиново-чёрную заглушку и напишет причину. У свежего клона так и будет,
    // `textures/` не в репозитории.
    //
    // Фотографию видно вблизи и крупно, то есть картинка растянута — это лечит
    // билинейное увеличение. Мип-уровни ей ни к чему: куб не уходит вдаль
    let photo = app
        .assets
        .load_texture("textures/noname.png", Magnify::Linear, Minify::Linear);

    // А пол — тот самый случай, ради которого настройки и разведены на две.
    //
    // Вблизи он растянут, и там нужны РЕЗКИЕ квадраты: шахматка на полу стоит
    // не для красоты, по ней видно перспективную коррекцию, а размытая она
    // ничего не показывает. Вдали он сжат до того, что в один пиксель
    // попадает пачка текселей, и там нужны мип-уровни: без них ближайший
    ...
    let deformed = app.assets.mesh(cube).clone();
    let deformed = app.assets.add_mesh(deformed);
    app.scene.instances[0].mesh = deformed;

    let mesh = app.assets.mesh_mut(deformed);
