...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let photo_image =
        Texture::decode(include_bytes!("../textures/noname.png")).unwrap_or_else(|err| {
            eprintln!("текстура не загрузилась ({err}), берём шахматку");
            // Замыкание только одалживает картинку — сама она уедет в пол ниже
            checker_image.clone()
        });

    // Фотографию на кубе видно вблизи и крупно: текселей на пиксель МЕНЬШЕ
    // одного, то есть картинка растянута, и лечится это билинейной выборкой.
    // Мип-уровни ей ни к чему — куб не уходит вдаль
    let photo = app
        .assets
        .add_texture(photo_image.with_filter(Magnify::Linear, Minify::Linear));
