...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // `Nearest` при растяжении и анизотропия при сжатии — ровно то сочетание,
    // ради которого две настройки и разведены: вблизи нужны РЕЗКИЕ квадраты
    // (шахматка на полу показывает перспективную коррекцию, размытая не
    // показывает ничего), вдали — спокойствие вместо ряби.
    //
    // Анизотропия, а не просто мип-уровни, — из-за пола: он единственный
    // объект сцены, который виден под скользящим углом, а там изотропная
    // выборка берёт уровень по ДЛИННОЙ стороне отпечатка и размывает клетки
    // поперёк взгляда зря. Та же настройка, что у пола в `demo.rs`.
    //
    // Просьба одна, исполнители разные: на CPU-пути выборки вдоль отпечатка
    // считает `Texture::sample_grad`, здесь — драйвер, по одному полю
    // `anisotropyEnable` у сэмплера. Пирамиду тоже строит сама видеокарта,
    // блитами (`vulkan::image::generate_mipmaps`)
    let checker = app.assets.add_texture(
        Texture::checker(64, 8, [230, 230, 230, 255], [40, 40, 60, 255])
            .with_filter(Magnify::Nearest, Minify::Anisotropic { max_samples: 8 }),
    );
