...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // `Nearest` при растяжении и `Mipmapped` при сжатии — ровно то сочетание,
    ...
    //
    // На GPU-пути пирамиду строит уже сама видеокарта, блитами
    // (`vulkan::image::generate_mipmaps`), а не `Texture::with_mipmaps`: её
    // уровни на видеокарту не едут вовсе. Просьба одна и та же, исполнители
    // разные — это и есть то, чего добивались мостом к сцене
