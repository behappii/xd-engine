...
/// или `&[u32]` как есть, не сериализуя вручную, а размер данных считается
/// из `size_of::<T>()`, а не угадывается снаружи.
///
/// `T: Copy` — граница слабее, чем хотелось бы, и важно понимать, чего она
/// НЕ обещает. Она отсекает типы с деструктором — `Vec`, `String`, `Box`, —
/// то есть ровно те, у которых байты значения это указатель на чужую
/// память, а копировать надо было бы содержимое. Но `Copy` — не признак
/// «внутри нет указателей»: `*const T` и `&'static T` тоже `Copy`, и такой
/// тип уехал бы на GPU бессмысленным адресом хостовой памяти. Настоящей
/// границы для «этот тип можно слать по проводу байтами» в самом языке нет
/// (в экосистеме её изображает `bytemuck::Pod`, отдельный крейт с
/// собственным `unsafe`); здесь ей взяться неоткуда, а вызывающих сторон
/// ровно три — `GpuVertex`, `u32`, `u8`
