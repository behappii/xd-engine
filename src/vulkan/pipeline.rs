...
/// MVP для позиции и матрица нормалей для освещения — вместе, потому что
/// это ОДИН push-constant блок в шейдере (`shader.rs`), а не два раздельных.
/// `#[repr(C)]` без паддинга между полями: оба — массивы `f32` с
/// выравниванием 4, и 112 байт получаются ровно теми же семью vec4-слотами
/// (offset 0, 16, …, 96), что в SPIR-V — считать `size_of` от одного типа,
/// а не от суммы двух чисел в двух файлах
#[repr(C)]
#[derive(Clone, Copy)]
