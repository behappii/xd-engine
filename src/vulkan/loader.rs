...
///
/// **Без `#[macro_export]`, и это принципиально.** Тот атрибут кладёт макрос
/// в КОРЕНЬ крейта и делает его публичным — причём молча, `pub` рядом с ним
/// не пишется, так что по объявлению этого не видно вовсе. `xd_engine`
/// пообещал бы наружу `xd_engine::vk_load!` — внутренний инструмент загрузки
/// FFI, которому в публичном API движка делать нечего (см. «Публичная
/// поверхность текстуры — семь вещей» в CLAUDE.md: критерий тот же — кто
/// знает ответ, тот и владеет; здесь ответ знает только сам модуль).
/// Правильный способ поделиться макросом внутри крейта — обычный
/// `macro_rules!` плюс `pub(crate) use` ниже: тогда он импортируется по пути
/// (`use crate::vulkan::loader::vk_load;`), как любой другой элемент, и
/// границы видимости у него ровно те же, что у функции
macro_rules! vk_load {
...
///
/// Не экспортируется наружу крейта по той же причине, что и `vk_load!` — см.
/// его doc-комментарий
