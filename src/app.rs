...
impl EngineApp {
    ...
    ///
    /// Потеря фокуса обязана отпустить ВСЁ. Причина в том, что события
    /// клавиатуры идут окну в фокусе: переключились по Alt-Tab с зажатым `W`
    /// — и отпускание уедет уже другому окну, а до нас не доедет никогда.
    /// Клавиша так и останется в множестве зажатых, и камера будет ехать сама
    /// по себе, пока не вернёшься и не нажмёшь-отпустишь `W` вручную.
    ///
    /// Обратное неверно: возврат фокуса не трогает ничего. Множество к этому
    /// моменту и так пустое, а что пользователь держит прямо сейчас, ОС
    /// доложит обычными событиями нажатия
...
mod tests {
    ...
    fn losing_focus_releases_every_held_key() {
        // Alt-Tab с зажатым W: события клавиатуры уходят окну в фокусе, значит
        // отпускание достанется уже другому окну, а до нас не доедет никогда.
        // Без этой очистки W оставался зажатым навсегда, и камера ехала сама
        // по себе, пока не вернёшься и не нажмёшь-отпустишь его вручную.
        //
        // Возврат фокуса проверяется тем же тестом, и не для симметрии:
        // перепутанное условие (чистить на ПОЛУЧЕНИИ фокуса) выглядит
        // правдоподобно, тоже «что-то очищает» — и оставляет исходный баг
        ...
        assert_eq!(
            app.pressed_keys.len(),
            2,
            "возврат фокуса не должен ничего отпускать"
        );

        app.focus_changed(false);
        assert!(
            app.pressed_keys.is_empty(),
            "после потери фокуса зажатых клавиш быть не может: {:?}",
            app.pressed_keys
        );
    }
}
