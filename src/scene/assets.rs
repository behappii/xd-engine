...
impl MeshId {
    /// Порядковый номер в арене — `pub(crate)`, не `pub`.
    ///
    /// Нужен GPU-бэкенду (`vulkan::gpu_assets`): он держит свою параллельную
    /// арену буферов, и индекс в ней совпадает с индексом здесь по
    /// построению, поэтому никакой карты соответствия не нужно — достаточно
    /// самого числа.
    ///
    /// Наружу не выставляется намеренно, по тому же критерию, что и вся
    /// остальная граница движка (CLAUDE.md, «кто знает ответ»): что `MeshId`
    /// внутри — просто индекс, это МЕХАНИЗМ, и обещать его стабильность
    /// пользователю нельзя. Именно из-за индекса `MeshId` из чужого `Assets`
    /// молча берёт не тот меш — эту особенность лечат поколениями, и тогда
    /// число перестанет быть просто числом
...
impl TextureId {
    /// То же, что [`MeshId::index`], и по тем же причинам
    pub(crate) fn index(self) -> usize {
        self.0
    }
}
...
impl Assets {
    ...
    /// Все меши подряд — `pub(crate)`, для GPU-бэкенда.
    ///
    /// Ему нужно не «взять меш по ссылке», а пройти арену целиком и завести
    /// вершинный буфер каждому. Обычного `mesh(id)` для этого мало: снаружи
    /// неоткуда узнать даже сколько их всего.
    ///
    /// Срез, а не итератор, ровно потому, что важна ДЛИНА: по разнице длин
    /// GPU-сторона и понимает, что появились новые меши и их надо догрузить
    /// (см. `vulkan::gpu_assets`). Арены только растут — `add_mesh` это
    /// `push`, — поэтому сравнения длин достаточно
